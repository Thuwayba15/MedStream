...
export const ClinicianConsultationProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const saveEncounterNoteDraft = useCallback(async (payload: ISaveEncounterNoteDraftRequest): Promise<IEncounterNote | null> => {
    ...
    const saveVitals = useCallback(async (payload: ISaveVitalsRequest): Promise<IConsultationVitalSigns | null> => {
    ...
    const attachTranscript = useCallback(async (payload: IAttachConsultationTranscriptRequest): Promise<IConsultationTranscript | null> => {
        dispatch(attachTranscriptStarted());
        try {
            const transcript = await postJson<IConsultationTranscript>(
                API.CLINICIAN_CONSULTATION_TRANSCRIPT_ROUTE,
                payload,
                "Unable to attach consultation transcript."
            );
            const workspace = await refreshWorkspace(payload.visitId);
    ...
    const generateSubjectiveDraft = useCallback(async (visitId: number): Promise<IConsultationAiDraft | null> => {
        ...
        try {
            const draft = await postJson<IConsultationAiDraft>(
                API.CLINICIAN_CONSULTATION_SUBJECTIVE_DRAFT_ROUTE,
                { visitId },
                "Unable to generate subjective draft."
            );
    ...
    const generateAssessmentPlanDraft = useCallback(async (visitId: number): Promise<IConsultationAiDraft | null> => {
        ...
        try {
            const draft = await postJson<IConsultationAiDraft>(
                API.CLINICIAN_CONSULTATION_ASSESSMENT_PLAN_DRAFT_ROUTE,
                { visitId },
                "Unable to generate assessment and plan draft."
            );
    ...
    const finalizeEncounterNote = useCallback(async (visitId: number): Promise<IEncounterNote | null> => {
